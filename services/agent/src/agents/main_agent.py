...
class MainAgent(BaseAgent):
    ...
    def __init__(self, llm_client: LiteLLMClient = None):
        ...
        self.logger.info(f"Initialized main agent with {len(self.sub_agents)} sub-agents")
    ...
    async def process(self, context: AgentContext) -> AgentResponse:
        ...
        self.logger.info(f"Main agent processing request for conversation: {context.conversation_id}")
        ...
        try:
            ...
            if response.success:
                self.logger.info(f"Successfully processed by {selected_agent.name} agent")
            else:
                self.logger.warning(f"Processing failed in {selected_agent.name} agent: {response.error_message}")
    ...
    async def _select_agent(self, context: AgentContext) -> Optional[BaseAgent]:
        ...
        """
        self.logger.debug(f"Selecting agent for message: {context.current_message[:100]}...")
