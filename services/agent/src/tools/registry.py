    ...
    async def initialize(self) -> bool:
        ...
        for tool_name, tool in self._tools.items():
            try:
                ...
                failed_tools.append(tool_name)
                self.logger.error(
                    "Tool initialization error", tool=tool_name, error=str(e)
                )

        if failed_tools:
            self.logger.warning(
                "Some tools failed to initialize", failed_tools=failed_tools
            )
            return False
    ...
    async def register_tool(self, tool: BaseTool) -> bool:
        ...
        try:
            if tool.name in self._tools:
                self.logger.warning(
                    "Tool already registered, replacing", tool=tool.name
                )

            self._tools[tool.name] = tool
            self.logger.info(
                "Tool registered", tool=tool.name, description=tool.description
            )
            ...
            if self._initialized:
                ...
                if not success:
                    self.logger.error(
                        "Failed to initialize newly registered tool", tool=tool.name
                    )
                    return False
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        for tool_name, tool in self._tools.items():
            try:
                ...
                self.logger.error(
                    "Error checking tool health", tool=tool_name, error=str(e)
                )
    ...
    async def cleanup(self) -> None:
        ...
        for tool_name, tool in self._tools.items():
            try:
                ...
                self.logger.error(
                    "Error cleaning up tool", tool=tool_name, error=str(e)
                )
