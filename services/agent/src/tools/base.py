...
class ToolCapability(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Required parameters"
    )
...
class MCPTool(BaseTool):
    ...
    async def _call_mcp_function(
        self, function_name: str, parameters: Dict[str, Any], timeout: int = 30
    ) -> ToolExecutionResult:
        ...
        try:
            self.logger.info(
                "Calling MCP function", function=function_name, parameters=parameters
            )
            ...
            result_output = (
                f"MCP function '{function_name}' called with parameters: {parameters}"
            )
            ...
            execution_time = time.time() - start_time
            self.logger.error(
                "Error calling MCP function", function=function_name, error=str(e)
            )
            return self._create_error_result(str(e), execution_time)
...
class FunctionTool(BaseTool):
    ...
    async def _call_function(
        ...
        try:
            ...
            self.logger.info(
                "Calling function", function=function_name, parameters=parameters
            )
            ...
            execution_time = time.time() - start_time
            return self._create_error_result(
                f"Function call timed out after {timeout} seconds", execution_time
            )
        except Exception as e:
            execution_time = time.time() - start_time
            self.logger.error(
                "Error calling function", function=function_name, error=str(e)
            )
            return self._create_error_result(str(e), execution_time)
