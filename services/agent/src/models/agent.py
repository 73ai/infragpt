...
class AgentRequest(BaseModel):
    ...
    past_messages: List[Message] = Field(
    ...
    channel_id: Optional[str] = Field(
        default=None, description="Channel/workspace information"
    )
...
class AgentResponse(BaseModel):
    ...
    agent_type: Optional[str] = Field(
        default=None, description="Type of agent that processed the request"
    )
    confidence: Optional[float] = Field(
        default=None, description="Confidence score (0.0 to 1.0)"
    )
    tools_used: List[str] = Field(
        default_factory=list, description="List of tools used in processing"
    )
