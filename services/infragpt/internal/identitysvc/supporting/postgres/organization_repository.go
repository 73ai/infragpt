...
func (r *organizationRepository) OrganizationByClerkID(ctx context.Context, clerkOrgID string) (*domain.Organization, error) {
	...
	if err == nil {
		useCases := make([]infragpt.UseCase, len(metadata.UseCases))
		for i, uc := range metadata.UseCases {
			useCases[i] = infragpt.UseCase(uc)
		}

		stack := make([]infragpt.ObservabilityStack, len(metadata.ObservabilityStack))
		...
		result.Metadata = domain.OrganizationMetadata{
			...
			CompanySize:        infragpt.CompanySize(metadata.CompanySize),
			TeamSize:           infragpt.TeamSize(metadata.TeamSize),
			UseCases:           useCases,
	...
	for i, org := range orgs {
		...
		if err == nil {
			...
			for j, uc := range metadata.UseCases {
				useCases[j] = infragpt.UseCase(uc)
			}

			stack := make([]infragpt.ObservabilityStack, len(metadata.ObservabilityStack))
			for j, s := range metadata.ObservabilityStack {
				stack[j] = infragpt.ObservabilityStack(s)
			}
