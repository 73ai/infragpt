...
export function SummaryStep({ form }: SummaryStepProps) {
  ...
  const getUseCaseLabels = (values: string[]) =>
    values.map(value => USE_CASES.find(uc => uc.value === value)?.label || value);

  const getObservabilityStackLabels = (values: string[]) =>
    values.map(value => OBSERVABILITY_STACK.find(tool => tool.value === value)?.label || value);
    ...
    <div className="space-y-6">
      <div className="rounded-lg border p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-medium text-sm text-muted-foreground mb-2">Company Size</h4>
            <p className="text-sm">{getCompanySizeLabel(values.companySize)}</p>
          </div>
          
          <div>
            <h4 className="font-medium text-sm text-muted-foreground mb-2">Team Size</h4>
        ...
        <div>
          <h4 className="font-medium text-sm text-muted-foreground mb-2">Current Observability Stack</h4>
          <div className="flex flex-wrap gap-2">
            {getObservabilityStackLabels(values.observabilityStack).map((label, index) => (
              <span 
                key={index}
      ...
      <div className="text-sm text-muted-foreground">
        Click "Complete Setup" to save your information and start using InfraGPT.
      </div>
    </div>
  );
}