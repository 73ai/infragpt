...
const onboardingSchema = z.object({
  companySize: z.enum(COMPANY_SIZES.map(c => c.value) as [string, ...string[]]),
  teamSize: z.enum(TEAM_SIZES.map(t => t.value) as [string, ...string[]]),
  useCases: z.array(z.enum(USE_CASES.map(u => u.value) as [string, ...string[]])).min(1, 'Please select at least one use case'),
  observabilityStack: z.array(z.enum(OBSERVABILITY_STACK.map(o => o.value) as [string, ...string[]])).min(1, 'Please select at least one tool'),
});
...
const STEPS = [
  ...
  { id: 3, title: 'Current Stack', description: 'What tools do you currently use?' },
